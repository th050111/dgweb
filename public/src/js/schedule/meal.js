  ...
  fetch(
    ...
    .then((myJson) => {
      localStorage.setItem(
        "meal-list",
        JSON.stringify(myJson["mealServiceDietInfo"][1].row)
      );
...
function drawList({ dateJson, list }) {
  const mealHtmlList = document.querySelectorAll(".meal-container>div");
  for (let i = 0; i < 3; i++) {
    ...
    for (let j = 0; j < list[i].length; j++) {
      ...
      const a = list[i][j]
        ...
        .replace(/[0-9]/g, "");
      div.innerHTML = a;
      if (mealCnt[a]) div.title = "올해의 " + mealCnt[a] + "번째 " + a;
      const br = document.createElement("br");
      if (plusDate == 44) div.style.color = "red";
      divList.appendChild(div);
    }
  ...
  if (plusDate <= 2)
    ...
    document.querySelector(".date").innerHTML =
      plusDate + "일 후의 급식" + "(" + getDay(dateJson) + ")";
  else if (plusDate == 44) {
    document.querySelector(".date").innerHTML =
      plusDate + "일 후의 급식" + "(" + getDay(dateJson) + ")";
...
function getCnt(dateJson) {
  ...
  mealList.forEach((meal) => {
    if (
      Number(meal.MLSV_YMD) >= 20220301 &&
      Number(meal.MLSV_YMD) <= Number(dateStr)
    ) {
...
function getDay(dateJson) {
  //날짜문자열 형식은 자유로운 편

  var week = ["일", "월", "화", "수", "목", "금", "토"];

  var dayOfWeek =
    week[new Date(dateJson.year, dateJson.month - 1, dateJson.date).getDay()];
